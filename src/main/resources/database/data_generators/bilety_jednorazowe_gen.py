...
for id_przejazdy_sklad_czesci in range(1, len(przejazdy_sklad_czesci)):
    ...
    for miniPrzejazd in range(start, end):
        if miniPrzejazd not in list(driver[id_przejazdu].keys()):
...
for i in range(1, len(bilety_jednorazowe_zamowienia)):
    if i % 1000 == 0:
        print(i)
    Losuj = True
    id_przejazdu, start, end, wagon, miejsce = random.randint(1, len(przejazdy) - 1), 0, 0, 0, 0
    while Losuj:
        Losuj = False
        ...
        if len(driver[id_przejazdu][start][wagon]) == 0:
            Losuj = True
            break
        miejsce = random.randint(1, len(list(driver[id_przejazdu][start][wagon].keys())))
        for miniPrzejazd in range(start, end):
